  ...
  try {
    ...
    for await (const chunk of stream) {
      ...
      if (chunk.delim === 'start') {
        ...
        process.stdout.write(chunk.content);
        // Check if the assistant's response mentions the weather info
        if (chunk.content.toLowerCase().includes('new york') || 
            chunk.content.includes('68°F') || 
            chunk.content.toLowerCase().includes('cloudy') ||
            chunk.content.toLowerCase().includes('humidity')) {
        ...
        console.log('\n📦 [FINAL RESPONSE RECEIVED]');
        // Also check in the final response
        const finalResponse = chunk.response;
        if (finalResponse.messages.some(msg => 
          msg.role === 'tool' && msg.content.includes('Current weather in New York'))) {
          toolCallExecuted = true;
        }
      }
    }
...
export { runComprehensiveStreamingTests, testResults, generateReport };
